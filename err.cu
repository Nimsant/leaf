...
int im3D_pars4save::init_from_command_line(char** argv) {
  ...
  else if(strcmp(*argv,"--devN")==0) cudaSetDevice(read_int(argv[1]));
  ...
  else if(strcmp(*argv,"--drop_dir")==0) set_drop_dir(argv[1]);
...
const char* im3D_pars4save::command_line_help_string() {
  return "[--devQ] [--devN <dev-num>] [--load <opt-file>] [--zoom \"1. 1. 1.\"] [--shrink \"1 1 1\"] [--step \"1. 1. 1.\"] [--base \"1. 1. 1.\"] [--box \"1. 1. 1.\"] [--mesh \"200. 200. 200.\"] [--Dmesh 5.] [--drop_dir \".\"] [--bkgr_col \"0.1 0.1 0.1\"] [--mesh_col \"0.8 0.8 0.2\"] [--box_col \"1. 1. 1.\"] [--box_shrink \"1. 1. 1.\"] [--sensor \"1 1 1\"]";
}
void im3D_pars4save::print_command_line_help() {
  printf("  --devQ\tВыдаёт информацию о видеокартах на компьютере;\n");
  printf("  --devN\tУстнавливает номер текущей видеокарты, 0;\n");
